    ...
    try:
        ...
        if operation == 'add':
            if 'a' not in data or 'b' not in data:
                raise ValueError("Missing operands for addition")
            result = data['a'] + data['b']
        elif operation == 'subtract':
            if 'a' not in data or 'b' not in data:
                raise ValueError("Missing operands for subtraction")
            result = data['a'] - data['b']
        elif operation == 'multiply':
            if 'a' not in data or 'b' not in data:
                raise ValueError("Missing operands for multiplication")
            result = data['a'] * data['b']
        elif operation == 'divide':
            if 'a' not in data or 'b' not in data:
                raise ValueError("Missing operands for division")
            if data['b'] == 0:
...
async def upload_file(
   ...
   try:
       ...
       project_id = os.getenv('PROJECT_ID', 'f8bbef17-24c5-493c-b35a-640f7dc50eb2')
